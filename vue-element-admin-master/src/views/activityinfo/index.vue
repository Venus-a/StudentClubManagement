<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="活动名称" width="150px">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="活动时间" width="250px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.acTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动地址" width="230px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          v-if="dialogStatus===update && flag==true"
          type="primary"
...
import item from '@/layout/components/Sidebar/Item.vue' // secondary package based on el-pagination
...
export default {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      ...
      // this.imageUrl = 'http://localhost:9999/team/fileUpload' + res.msg
    },
    ...
    getList() {
       ...
       * 3把加载狂取消
       */
      this.listLoading = true // 没有信息之前显示加载框.
      request(
        {
    ...
    handleDelete(row, index) {
       ...
       *1获取要删除记录的id 主键
       * 2发送请求给后端服务器
       * 3前后端根据后端服务器返回结果判断是否成功
       * 4成功的话 重新加载table  展示最新内容
       */
